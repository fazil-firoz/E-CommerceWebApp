...
const CouponManagement = () => {
  ...
  const handleToggleActive = async (record, checked) => {
    try {
      ...
      if (res.success) {
        message.success(
          `Coupon ${record.code} ${checked ? "enabled" : "disabled"}`,
        );
        fetchCoupons();
      } else {
  ...
  const activeCount = coupons.filter(
    (c) => c.isActive && dayjs(c.expiryDate).isAfter(dayjs()),
  ).length;
  const expiredCount = coupons.filter((c) =>
    dayjs(c.expiryDate).isBefore(dayjs()),
  ).length;
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "14px" }}>
          ...
          <div>
            ...
            <Text type="secondary" style={{ fontSize: "13px" }}>
              Create and manage promotional discount coupons, minimum purchase
              rules, and expiry dates.
            </Text>
          </div>
        </div>
        ...
        >
          <Form.Item
            ...
            rules={[
              ...
              {
                pattern: /^[A-Za-z0-9_-]+$/,
                message: `Code must be alphanumeric (e.g. ${shopPrefix}10)`,
              },
            ]}
          >
            <Input
              placeholder={`e.g. ${shopPrefix}10`}
              size="large"
              style={{
